...
export class InsurerLayout implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private insuranceApi: InsuranceService,
  ) {}
  ...
  ngOnInit(): void {
    ...
    this.applyTheme();
    this.loadUser();
    this.insuranceApi.getProducts().subscribe({
      next: (list) => {
        if (!list?.length) return;
        const types: Array<'auto' | 'habitation' | 'sante' | 'vie'> = ['auto', 'habitation', 'sante', 'vie'];
        this.offers = list.map((p, i) => ({
          id: p.id,
          name: p.name,
          type: types[i % types.length],
          price: Math.max(0, Math.round(Number(p.baseRatePct) * 50)),
          duration: '12 mois',
          coverage: (p.description && p.description.slice(0, 80)) || 'Micro-insurance product',
          status: p.status === 'ACTIVE' || p.status === 'AVAILABLE' ? 'active' : 'draft',
          subscribers: 0,
