...
export class BackofficeComponent implements OnInit, OnDestroy {
  ...
  /** Prevents infinite “Chargement…” when the backend never responds. */
  ...
  loadClients(): void {
    ...
    this.http
      ...
      .pipe(
        ...
        catchError((err) => {
          console.error('[Backoffice] loadClients failed or timed out', err);
          return of([] as any[]);
        }),
        finalize(() => {
  ...
  loadLogs(): void {
    ...
    this.http
      .get<any[]>(`${this.API}/users/logs`)
      .pipe(
        timeout(this.httpTimeoutMs),
        catchError((err) => {
          console.error('[Backoffice] loadLogs failed or timed out', err);
          return of([] as any[]);
        }),
        finalize(() => {
      ...
      .subscribe({
        next: (logs) => {
          this.logsList = Array.isArray(logs) ? logs : [];
        },
      });
  }
  ...
  loadUsers(): void {
    ...
    this.http
      ...
      .pipe(
        timeout(this.httpTimeoutMs),
        catchError((err) => {
          console.error('[Backoffice] loadUsers failed or timed out', err);
          return of([] as any[]);
        }),
        finalize(() => {
  ...
  selectAdminClient(c: any): void {
    ...
    this.http
      .get<any>(`${this.API}/payment-history/next-installment/by-user/${c.id}`)
      .pipe(
        timeout(this.httpTimeoutMs),
        finalize(() => {
          this.adminInstLoading = false;
  ...
  loadAdminPayments(): void {
    ...
    this.http
      .get<any[]>(`${this.API}/payment-history/admin/all`)
      .pipe(
        timeout(this.httpTimeoutMs),
        catchError((err) => {
        ...
        finalize(() => {
          this.adminPaymentsLoading = false;
          this.cdr.detectChanges();
        }),
      )
      .subscribe({
        next: (res) => {
          this.adminPayments = Array.isArray(res) ? res : [];
        },
      });
  }
