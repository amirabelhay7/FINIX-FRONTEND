...
export class AdvancedIndicatorsComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  ngAfterViewChecked(): void {
    if (this.data && !this.built) this.buildCharts();
  }

  buildCharts(): void {
    if (!this.data || !this.yieldRef || !this.gainRef || !this.clientRef) return;
    this.built = true;
      ...
      options: {
        ...
        scales: {
          y: {
            beginAtZero: true,
            ticks: { callback: v => v + '%' },
            // ligne de seuil 15% via annotation ou repère visuel
          }
        }
      }
    }));
    ...
    // ── 3. Client Growth Chart (combo bar + line) ────────────────────────────
